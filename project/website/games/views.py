...
def _build_stat(games, players, room_filter="all"):
    ...
    average_deal_scores = rounds.filter(is_deal=True).aggregate(Avg("lose_scores"))["lose_scores__avg"]
    average_win_scores = rounds.filter(is_win=True).aggregate(Avg("win_scores"))["win_scores__avg"]
    average_game_scores = games.aggregate(Avg("scores"))["scores__avg"]
    places = games.values_list("player_position", flat=True)
    dan_3_pt = sum(
        [60 for x in places if x == 1] + [15 for x in places if x == 2] + [-75 for x in places if x == 4]
    )
